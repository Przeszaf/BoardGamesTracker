...
class AllGamesBarView: UIView {
    ...
    convenience init(frame: CGRect, dataSet: [Int], dataName: [String]) {
        ...
        pieChart = PieChartView(dataSet: dataSet, dataName: dataName, radius: 80, frame: CGRect.init(x: 0, y: 25, width: frame.width, height: frame.height - 40), truncating: 12, colorsArray: nil)
        addSubview(pieChart)
    }
